    ...
    public class ExpeditionManager
    {
        ...
        public class Expedition
        {
            ...
            public void UpdateTasks()
            {
                ...
                foreach (ExpeditionTask Task in m_Tasks)
                {
                    if (!DontUpdateLaterTasks)
                    {
                        ...
                        if ((CompleteOrder == ExpeditionCompleteOrder.LINEAL || CompleteOrder == ExpeditionCompleteOrder.LINEALHIDDEN || CompleteOrder == ExpeditionCompleteOrder.LINEALLAST) && !Task.m_IsComplete) // If Lineal do not update later tasks.
                        {
                    ...
                    if (CompleteOrder == ExpeditionCompleteOrder.LINEALLAST)
                    {
                        ...
                        {
                            Text = ColorPrefix + Task.m_Text + ColorAffix;
                            HideNextOnes = true;
                        }
                    } else if (CompleteOrder == ExpeditionCompleteOrder.LINEAL || CompleteOrder == ExpeditionCompleteOrder.ANYORDER || CompleteOrder == ExpeditionCompleteOrder.LINEALHIDDEN)
                    {
                        if (m_Tasks.Count > 1)
                        {
                            if(!HideNextOnes)
                            {
                                ...
                                {
                                    TaskText += Task.m_Text + ColorAffix;
                                } else
                                {
                                    TaskText = "\n" + TaskText + Task.m_Text + ColorAffix;
                                }
        ...
        public static void RegisterFlaregunShot(string GUID, Vector3 Position)
        {
            DebugLog("[RegisterFlaregunShot] "+ GUID);
            for (int i = m_ActiveExpeditions.Count - 1; i > -1; i--)
            {
                for (int i2 = m_ActiveExpeditions[i].m_Tasks.Count - 1; i2 > -1; i2--)
                {
                    ...
                    {
                        m_ActiveExpeditions[i].m_Tasks[i2].CheckFlaregunShot(GUID, Position);
                    }
                }
            }
        }
        ...
        public class ExpeditionTask
        {
            ...
            public void CheckFlaregunShot(string GUID, Vector3 Position)
            {
                if (!m_IsComplete && m_CanCheckFlaregun)
                {
                    if (m_Type == ExpeditionTaskType.FLAREGUNSHOT)
                    {
                        if (GUID == m_Scene && Vector3.Distance(Position, m_ZoneCenter) <= m_ZoneRadius)
                        {
                            OnCompleted();
                            return;
                        }
